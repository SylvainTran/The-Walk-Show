...
public class Combatant : Bot
{
    ...
    public override Vector3 RandomizeWanderParameters()
    {
        ...
        while (attempts < 5)
        {
            ...
            wanderTarget = parent.position + transform.TransformDirection(localWander);
    ...
    {
        Collider[] hitColliders = Physics.OverlapBox(parent.position, parent.localScale / 2, Quaternion.identity);
        int i = 0;
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            if (Vector3.Distance(chasedTarget.transform.position, parent.position) >= chaseRange)
            {
                chasedTarget = null;
    ...
    {
        Debug.Log($"{parent.gameObject.name}  dealt {damage} damage to {opponent.gameObject.name}");
